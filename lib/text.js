        ...
        toWriteTextObjects.some((toWriteTextObject, index) => {
            ...
            const addHighlight = (x, y, ctx, options) => {
                ...
                if (options.highlight) {
                    ctx.drawRectangle(x, y, options.lineWidth, options.textHeight, {
                        // useGivenCoords: true,
                        // rotation: pathOptions.rotation,
                        // rotationOrigin: [pathOptions.originX, pathOptions.originY],
                        type: 'fill',
                        fill: options.hilite.color || '#ffff00',
                        opacity: options.hilite.opacity || 0.5
                    });
                }
            }
    ...
    const writeValue = (textObject) => {
        ...
        if (textObject.tag && textObject.childs.length) {
            ...
            textObject.childs.forEach((child) => {
                ...
                child.highlight = (textObject.highlight) ? textObject.highlight : child.highlight;
